...
describe('SettingsView', () => {
  ...
  it('handles workspace directory change', async () => {
    ...
    ;(AppModule.SelectWorkspaceDirectory as jest.MockedFunction<typeof AppModule.SelectWorkspaceDirectory>).mockResolvedValue(newPath)
  ...
  it('does not change workspace when SelectWorkspaceDirectory returns empty', async () => {
    const user = userEvent.setup()
    ;(AppModule.SelectWorkspaceDirectory as jest.MockedFunction<typeof AppModule.SelectWorkspaceDirectory>).mockResolvedValue('')
  ...
  it('handles SelectWorkspaceDirectory error gracefully', async () => {
    ...
    ;(AppModule.SelectWorkspaceDirectory as jest.MockedFunction<typeof AppModule.SelectWorkspaceDirectory>).mockRejectedValue(new Error('Directory selection failed'))
  ...
  it('validates workspace before saving', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Invalid workspace directory. Please ensure the path exists and is writable.')).toBeInTheDocument()
    })
  })
  ...
  it('handles save configuration error', async () => {
    ...
    const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => {})
    ;(AppModule.ValidateWorkspacePath as jest.MockedFunction<typeof AppModule.ValidateWorkspacePath>).mockResolvedValue(undefined)
    ;(AppModule.SaveConfig as jest.MockedFunction<typeof AppModule.SaveConfig>).mockRejectedValue(new Error('Save failed'))
  ...
  it('disables save button when validating', async () => {
    ...
    ;(AppModule.ValidateWorkspacePath as jest.MockedFunction<typeof AppModule.ValidateWorkspacePath>).mockImplementation(() => new Promise(() => {}))
  ...
  it('clears validation error when workspace path changes', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('Invalid workspace directory. Please ensure the path exists and is writable.')).not.toBeInTheDocument()
    })
  })
})